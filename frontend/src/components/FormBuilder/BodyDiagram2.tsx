...
<svg xmlns="http://www.w3.org/2000/svg" width="400" height="400" version="1.0" viewBox="0 0 300 300" style="max-width: 100%; height: auto; transform: scale(1.22); transform-origin: center;">
...
export const BodyDiagram2: React.FC<BodyDiagram2Props> = ({
  ...
  const handleSvgClick = (event: React.MouseEvent<HTMLDivElement>) => {
    ...
    // Calculate position considering the CSS scale transform
    const clickX = event.clientX - svgRect.left;
    const clickY = event.clientY - svgRect.top;

    const unscaledWidth = svgRect.width / scale;
    const unscaledHeight = svgRect.height / scale;

    const emptySpaceX = (svgRect.width - unscaledWidth) / 2;
    const emptySpaceY = (svgRect.height - unscaledHeight) / 2;

    const relativeX = clickX - emptySpaceX;
    const relativeY = clickY - emptySpaceY;

    let x = (relativeX / unscaledWidth) * 100;
    let y = (relativeY / unscaledHeight) * 100;

    // Clamp values to be within the 0-100 range
